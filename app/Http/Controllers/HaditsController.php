...
class HaditsController extends Controller
{
  ...
  public function result(Request $request)
  {
    ...
    //------------------------Preprocessing hadits-------------------------------------------//
        ...
        $bank_of_words = array();
        foreach ($pt_haditss as $key => $pt_hadits) {
          $bank_of_words[$key] = array_count_values(str_word_count($pt_hadits->isi, 1));
        }



        // dd($bank_of_words, $daftar_kata);

        foreach ($daftar_kata as $key => $kata) {
          $df[$key]=0;
          foreach ($bank_of_words as $ayat => $words) {
            foreach ($words as $qq => $value) {
              // dd($words,$bank_of_words,$daftar_kata);
              if($kata == $qq){
                // dd($key, $ayat, $qq, $bank_of_words[$ayat][$qq]);
                // dd($value);
                $tf[$key][$ayat] = $value;
                // dd($kata, $key, $ayat, $value);
                $df[$key]++;
                 // print_r("$kata tf[$key]] | ayat = ".($ayat+1)." | jumlah =   <br> ");
              }
            }
          }
          // dd($tf);
          $idf[$key] = number_format(log($jumlah_hadits/$df[$key] , 10), 2);
          if(in_array($kata, $query)){
            $Qkuadrat[$key] = number_format(pow($idf[$key], 2), 2);
          }
        }
        ...
        foreach ($daftar_kata as $key => $kata) {
          foreach ($haditss as $ayat => $hadits) {
            if(@$tf[$key][$ayat]){
              ...
              $Wkuadrat[$key][$ayat] = number_format(pow(($W[$key][$ayat]),2), 2);
              if(in_array($kata, $query)){
                $Q[$key][$ayat] = number_format(($idf[$key]*$W[$key][$ayat]), 2);
              }
            }
          }
        }
        ...
        foreach ($daftar_kata as $key => $kata) {
          foreach ($haditss as $ayat => $hadits) {
            if(@$Wkuadrat[$key][$ayat]){
              if(!is_numeric($Wkuadrat[$key][$ayat])){
                 $Wkuadrat[$key][$ayat] = str_replace(',', '', $Wkuadrat[$key][$ayat]);
              }
              if(!is_numeric($sumWkuadrat[$ayat])){
                // dd($sumWkuadrat[$ayat]);
                $sumWkuadrat[$ayat] = str_replace(',', '', $sumWkuadrat[$ayat]);
              }
              $sumWkuadrat[$ayat] = number_format(($sumWkuadrat[$ayat]+$Wkuadrat[$key][$ayat]),2);   
            }
          }
        }

        foreach ($sumWkuadrat as $key => $value) {
          if(!is_numeric($sumWkuadrat[$key])){
                // dd($sumWkuadrat[$key]);
                $sumWkuadrat[$key] = str_replace(',', '', $sumWkuadrat[$key]);
              }
          $root_sumWkuadrat[$key] = number_format(sqrt($sumWkuadrat[$key]), 2);
        }
        ...
        // dd($cosine);
        $flip_cosine = array_flip($cosine); //menukar $key dengan $value
        foreach ($haditss as $ayat => $hadits) {
          if(!array_search($ayat, $flip_cosine)){
            $cosine[$ayat] = 0;
          }
        }
        ...
        $ranking_cosine = $cosine;
        arsort($ranking_cosine);
        // dd($ranking_cosine);

        // $ranking_hadits = array();
        // // dd($cosine, $ranking_cosine);
        // // dd($daftar_kata,$tf,$df,$idf,$W,$Qkuadrat,$Wkuadrat,$Q,$sumWkuadrat);

        // foreach ($cosine as $ayat => $cos) {
        //   $ranking_hadits[$ayat]= array_search($cos, $ranking_cosine);
        // }
        // // dd($ranking_hadits);

        // $hadits_ranked = array();
        // foreach ($haditss as $key => $hadits) {
        //   foreach ($ranking_hadits as $ayat => $rank) {
        //     if($key == $ayat){
        ...
        // }
        // foreach ($ranking_hadits as $ayat => $rank) {
        //   $hadits_ranked[$rank] = $haditss[$ayat];
        //   $hadits_ranked[$rank]['cosine'] = $cosine[$ayat];
        // }

        // // $hadits_ranked = array_values($hadits_ranked);
        // dd($hadits_ranked);
        

       
         return view('admin.haditss.result', compact('kata_yang_dicari','query_implode','daftar_kata','haditss','jumlah_hadits','tf','df','idf','W','Wkuadrat','Q','Qkuadrat','sumQkuadrat','sumWkuadrat','root_sumWkuadrat','root_sumQkuadrat','cosine','ranking_cosine'));
